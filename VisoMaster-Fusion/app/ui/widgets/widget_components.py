...
class TargetMediaCardButton(CardButton):
    ...
    def load_media(self):
        ...
        if self.file_type == "video":
            ...
            was_fallback = False
            try:
                media_capture = cv2.VideoCapture(self.webcam_index, backend_id)
            except Exception as e:
                print(f"[WARN] Exception during webcam load init: {e}")
                media_capture = None

            if not media_capture or not media_capture.isOpened():
                print(f"[WARN] Failed to load webcam {self.webcam_index} with backend {backend_name}. Retrying with CAP_ANY...")
                was_fallback = True
                if media_capture:
                    ...
                    time.sleep(0.5)
                try:
                    media_capture = cv2.VideoCapture(self.webcam_index, cv2.CAP_ANY)
                except Exception as e:
                    print(f"[ERROR] Exception during webcam load init with CAP_ANY: {e}")
                    media_capture = None

            if media_capture and media_capture.isOpened():
                try:
                ...
                main_window.video_processor.fps = media_capture.get(cv2.CAP_PROP_FPS)
                main_window.video_processor.max_frame_number = max_frames_number
                main_window.video_processor._active_webcam_index = self.webcam_index
                main_window.video_processor._active_webcam_backend = backend_id
            else:
                frame = None
                max_frames_number = 0
