...
def extract_frame_as_image(
    ...
    if file_type == "image":
        ...
        try:
            ...
            print(f"[WARN] Exception during webcam thumbnail init: {e}")
            camera = None

        if not camera or not camera.isOpened():
            print(f"[WARN] Failed to open webcam thumbnail {webcam_index} with backend. Retrying with CAP_ANY...")
            if camera:
                camera.release()
                _time.sleep(0.5)
            try:
                camera = cv2.VideoCapture(webcam_index, cv2.CAP_ANY)
            except Exception as e:
                print(f"[ERROR] Exception during webcam thumbnail init with CAP_ANY: {e}")
                camera = None
