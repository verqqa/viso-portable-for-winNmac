...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def initialize_widgets(self):
        ...
        # Dynamically insert Browse button next to the Background Image Path setting field
        bg_path_widget = self.parameter_widgets.get("WebcamBackgroundImagePathText")
        if bg_path_widget and getattr(bg_path_widget, "row_widget", None):
            row_layout = bg_path_widget.row_widget.layout()
            if row_layout:
                ...
                def browse_bg_image():
                    ...
                    if file_path:
                        bg_path_widget.setText(file_path)
                        # Trigger the returnPressed signal to save the value in self.control
                        bg_path_widget.returnPressed.emit()
                browse_btn.clicked.connect(browse_bg_image)
                # Insert the Browse button before the reset default button (index 2)
                row_layout.insertWidget(2, browse_btn)
    ...
    def _initialize_target_videos_filter_menu(self):
        ...
        self.targetVideosFilterWebcamsCheckBox = (
            self._create_target_videos_filter_menu_checkbox(
                "Webcams", ":/media/media/webcam.png", checked=True
            )
        )
    ...
    def __init__(self, gpu_id=0):
        ...
        QtCore.QTimer.singleShot(800, self.load_last_workspace)
