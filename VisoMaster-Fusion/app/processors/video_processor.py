...
class VideoProcessor(QObject):
        ...
        while self.processing:
            try:
                ...
                if not ret:
                    ...
                    print(f"[WARN] Feeder: Failed to read webcam frame (failure {consecutive_failures}/20).")
                    if consecutive_failures >= 20:
                        print("[ERROR] Feeder: Webcam disconnected. Stopping webcam feed.")
                        self.main_window.display_messagebox_signal.emit(
                            "Webcam Disconnected",
                            "Webcam connection has been lost. Stopping the video/audio processing stream.",
                            self.main_window,
                        )
                        self.stop_processing()
                        break
                    time.sleep(0.05)  # Avoid pegging CPU at 100% in a tight retry loop
                    continue
    ...
    def stop_processing(self) -> bool:
        ...
        if self.feeder_thread and self.feeder_thread.is_alive():
            if threading.current_thread() != self.feeder_thread:
                ...
                print("[INFO] stop_processing called from within the feeder thread itself. Skipping join to prevent self-deadlock.")
        ...
        if self.file_type == "video" and self.media_path:
            ...
            try:
                ...
                try:
                    ...
                    print(f"[WARN] Exception during webcam init with backend {backend_name}: {e}")
                    self.media_capture = None

                if not self.media_capture or not self.media_capture.isOpened():
                    print(f"[WARN] Failed to open webcam {webcam_index} with backend {backend_name}. Retrying with CAP_ANY...")
                    was_fallback = True
                    if self.media_capture:
                        self.media_capture.release()
                        time.sleep(0.5)
                    try:
                        self.media_capture = cv2.VideoCapture(webcam_index, cv2.CAP_ANY)
                    except Exception as e:
                        print(f"[ERROR] Exception during webcam init with CAP_ANY: {e}")
                        self.media_capture = None

                if self.media_capture and self.media_capture.isOpened():
                    try:
                        if backend_id == cv2.CAP_DSHOW and not was_fallback:
    ...
    def process_webcam(self):
        ...
        current_active_idx = getattr(self, "_active_webcam_index", None)
        current_active_backend = getattr(self, "_active_webcam_backend", None)

        if (
            self.media_capture
            and self.media_capture.isOpened()
            and current_active_idx == webcam_index
            and current_active_backend == backend_id
        ):
            print("[INFO] Reusing already opened webcam capture.")
            was_fallback = (current_active_backend == cv2.CAP_ANY)
        else:
            if self.media_capture:
                misc_helpers.release_capture(self.media_capture)
                self.media_capture = None
        ...
        if not (self.media_capture and self.media_capture.isOpened()):
            ...
            self.main_window.display_messagebox_signal.emit(
                "Webcam Error",
                f"Unable to open the selected webcam (Index: {webcam_index}).\n\n"
                "Please verify that the camera is plugged in, not currently in use by another application "
                "(e.g., Zoom, OBS, Discord), and that the correct device index/backend is selected in Settings.",
                self.main_window,
            )
            video_control_actions.reset_media_buttons(self.main_window)
            return

        self._active_webcam_index = webcam_index
        self._active_webcam_backend = backend_id if not was_fallback else cv2.CAP_ANY

        # 3. Apply Configuration
        try:
            # Force MJPG to allow high framerate at high res (saves USB bandwidth) on DirectShow
            if backend_id == cv2.CAP_DSHOW and not was_fallback:
                fourcc = cv2.VideoWriter_fourcc(*"MJPG")
                self.media_capture.set(cv2.CAP_PROP_FOURCC, fourcc)
        except Exception:
