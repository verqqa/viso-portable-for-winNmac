...
class FrameWorker(threading.Thread):
    ...
    def __init__(
        ...
        # Background replacement cache
        self._cached_bg_path = ""
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    if self.video_processor.file_type == "webcam" and not self.is_single_frame:
                        self.video_processor.webcam_frame_processed_signal.emit(fallback_bgr)
                    elif not self.is_single_frame:
                        self.video_processor.frame_processed_signal.emit(
    ...
    def apply_background_replacement(self, img_rgb: np.ndarray, control: dict) -> np.ndarray:
        import os
        import cv2

        bg_path = control.get("WebcamBackgroundImagePathText", "")
        if not bg_path or not os.path.exists(bg_path):
            return img_rgb  # No valid background path, return original frame

        # Cache the background image
        if self._cached_bg_path != bg_path or self._cached_bg_image is None:
            try:
                # cv2.imread loads BGR, convert to RGB
                bg_img = cv2.imread(bg_path)
                if bg_img is not None:
                    self._cached_bg_image = cv2.cvtColor(bg_img, cv2.COLOR_BGR2RGB)
                    self._cached_bg_path = bg_path
                else:
                    return img_rgb
            except Exception as e:
        ...
        # Resize background image to match the frame size if needed
        h, w = img_rgb.shape[:2]
        bg_resized = self._cached_bg_image
        if bg_resized.shape[0] != h or bg_resized.shape[1] != w:
            bg_resized = cv2.resize(bg_resized, (w, h), interpolation=cv2.INTER_LINEAR)

        # 1. Convert source frame to HSV space
        hsv = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2HSV)

        # 2. Determine Key Color Range in HSV
        color_type = control.get("WebcamChromaKeyColorSelection", "Green")
        tolerance = int(control.get("WebcamChromaKeyToleranceSlider", 30))

        # Default HSV ranges
        # OpenCV HSV ranges: H: 0-180, S: 0-255, V: 0-255
        if color_type == "Green":
            # Pure green is H=120 in 360 deg, which is H=60 in OpenCV.
            key_h, key_s, key_v = 60, 200, 200
        elif color_type == "Blue":
            # Pure blue is H=240 in 360 deg, which is H=120 in OpenCV.
            key_h, key_s, key_v = 120, 200, 200
        else:  # Custom Hex
            hex_str = control.get("WebcamChromaKeyColorHexText", "#00FF00").lstrip('#')
            try:
                # Convert Hex to RGB
                ...
                key_h = int(hsv_pixel[0, 0, 0])
                key_s = int(hsv_pixel[0, 0, 1])
                key_v = int(hsv_pixel[0, 0, 2])
            except Exception:
                # Fallback to green
                key_h, key_s, key_v = 60, 200, 200

        # Create HSV mask bounds
        # H is circular (0-180), handle boundaries
        lower_h = (key_h - tolerance) % 180
        upper_h = (key_h + tolerance) % 180

        # Standard simple HSV thresholding:
        # Green/Blue screen is usually well saturated (S > 50) and not too dark (V > 50)
        min_s = 50
        min_v = 50

        if lower_h < upper_h:
            mask = cv2.inRange(hsv, np.array([lower_h, min_s, min_v]), np.array([upper_h, 255, 255]))
        else:
            # Wrap around case (e.g. Red which is around 0/180)
            mask1 = cv2.inRange(hsv, np.array([0, min_s, min_v]), np.array([upper_h, 255, 255]))
            mask2 = cv2.inRange(hsv, np.array([lower_h, min_s, min_v]), np.array([180, 255, 255]))
            mask = cv2.bitwise_or(mask1, mask2)

        # 3. Apply Edge Feathering (blur the mask)
        feather = int(control.get("WebcamChromaKeyFeatherSlider", 5))
        if feather > 0:
        ...
        # Convert mask to 0.0 - 1.0 float range
        mask_f = mask_blurred.astype(float) / 255.0
        # Expand dimensions to [H, W, 1] for broadcasting
        mask_f = np.expand_dims(mask_f, axis=2)

        # 4. Blend: output = background * mask + source * (1 - mask)
        blended = bg_resized * mask_f + img_rgb * (1.0 - mask_f)
    ...
    def process_frame(self, control: dict, stop_event: threading.Event):
        ...
        if control.get("WebcamBackgroundEnableToggle", False):
            img_numpy_rgb_uint8 = self.apply_background_replacement(img_numpy_rgb_uint8, control)
