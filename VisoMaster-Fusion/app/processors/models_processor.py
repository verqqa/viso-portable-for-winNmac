...
class ModelsProcessor(QtCore.QObject):
    ...
    def __init__(self, main_window: "MainWindow", device="cuda"):
        ...
        if self.gpu_id != 0 and device != "cpu" and is_cuda_avail:
    ...
    def load_model(self, model_name, session_options=None):
        ...
        with self.model_lock:
            ...
            # Blacklist for models that fail to compile, crash GPU driver, or have dynamic shapes
            trt_blacklisted_models = {
                # --- Face Restorers ---
                "GFPGANv1.4",
                "GFPGAN1024",
                "CodeFormer",
            ...
            is_blacklisted_from_trt = model_name in trt_blacklisted_models

            for p in self.providers:
                p_name = p[0] if isinstance(p, tuple) else p
                if p_name == "TensorrtExecutionProvider":
                    if is_blacklisted_from_trt:
                        print(f"[INFO] Bypassing TensorRT provider for '{model_name}' (blacklisted due to dynamic shapes / compilation issues). Falling back to next available provider.")
                        continue
                ...
                if is_tensorrt_load:
                    ...
                    if not cache_is_valid:
                        ...
                        try:
                            ...
                            print(f"[WARN] Isolated TensorRT probe failed for {model_name}: {e}")
                            print(f"[WARN] Falling back to non-TensorRT providers for '{model_name}'.")
                            
                            # Strip TensorRT execution provider from model_providers
                            model_providers = [
                                p for p in model_providers
    ...
    def switch_providers_priority(self, provider_name):
        ...
        # Downgrade to CPU if CUDA/NVIDIA is not available on this computer
        if not torch.cuda.is_available() and provider_name in ["TensorRT", "TensorRT-Engine", "CUDA"]:
            print(f"[WARN] CUDA is not available on this system. Downgrading {provider_name} to CPU.")
            provider_name = "CPU"
