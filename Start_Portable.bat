    ...
    if !ERRORLEVEL! neq 0 (
        ...
        if "!LOCAL!" neq "!REMOTE!" (
            ...
            if !ERRORLEVEL! equ 1 (
                ...
                popd
                "%APP_PYTHON%" -c "import pathlib; p=pathlib.Path(r'%CONFIG_FILE%'); c=p.read_text(encoding='utf-8') if p.is_file() else ''; lines=[l for l in c.splitlines() if not l.startswith('DOWNLOAD_RUN=')]; lines.append('DOWNLOAD_RUN=false'); p.write_text('\n'.join(lines), encoding='utf-8')"

                :: SELF-UPDATE CHECK DISABLED FOR AUTOMATION
                REM call :self_update_check
            ) else (
                popd
            )
        ) else (
...
if /I "!DOWNLOAD_RUN!"=="false" (
    ...
    if !ERRORLEVEL! equ 0 (
        "%APP_PYTHON%" -c "import pathlib; p=pathlib.Path(r'%CONFIG_FILE%'); c=p.read_text(encoding='utf-8') if p.is_file() else ''; lines=[l for l in c.splitlines() if not l.startswith('DOWNLOAD_RUN=')]; lines.append('DOWNLOAD_RUN=true'); p.write_text('\n'.join(lines), encoding='utf-8')"
    )
    popd
)
...
:self_update_check
    ...
    powershell -Command "Invoke-WebRequest -Uri '%REMOTE_URL%' -OutFile '%REMOTE_BAT%' -UseBasicParsing"
    if !ERRORLEVEL! neq 0 (
...
:install_dependency
    ...
    powershell -Command "Invoke-WebRequest -Uri '%URL%' -OutFile '%ZIP_FILE%' -UseBasicParsing"
...
:install_python
    ...
    if !WIN_BUILD! LSS 22000 (
        ...
        powershell -Command "Invoke-WebRequest -Uri '%PYTHON_NUGET_URL%' -OutFile '%PYTHON_NUGET_ZIP%' -UseBasicParsing"
        ...
        echo Downloading Python...
        powershell -Command "Invoke-WebRequest -Uri '%PYTHON_EMBED_URL%' -OutFile '%PYTHON_ZIP%' -UseBasicParsing"
        if !ERRORLEVEL! neq 0 ( echo ERROR: Failed to download Python. && pause && exit /b 1 )
        echo Extracting Python...
        ...
        for %%f in ("%PYTHON_DIR%\python*._pth") do (
            set "PTH_FILE=%%f"
            echo Enabling site packages in PTH file: %%f
            powershell -Command "^(Get-Content '%%f'^) -replace '#import site', 'import site' | Set-Content '%%f'"
            echo Adding portable repo path to PTH file...
            powershell -Command ^
                "$pth = Get-Content '%%f';" ^
                "if ^(-not ^($pth -match '\.\./\.\./VisoMaster-Fusion'^)^) {" ^
                "    Add-Content '%%f' ' ../../VisoMaster-Fusion';" ^
                "}"
        )
    )
